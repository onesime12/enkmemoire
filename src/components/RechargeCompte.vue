<template>
  <v-container>
    <v-card max-width="450" class="mx-auto">
      <v-toolbar color="blue" dark>
        ...
        <v-form @submit.prevent="recheachOperation" class="">
          <v-container>
            <v-row>
              <v-col cols="8" class="">
                <v-text-field class="" v-model="compteur" label="Numéro Compteur : ">
                </v-text-field>
              </v-col>
              <v-col cols="4" class="">
      ...
      <div v-if="vueSelf==false" class="h-60 d-flex justify-center align-center">
        <h1 class=" text-center text-gray-400 m-auto">
          Faites une recherche et le resultat s'affiche ici</h1>
      </div>
      <div v-else class="bg-gray-200 w-200 h-200" >
      <v-list v-for="(item,index) in gets" :key="index">
        <v-list-item class="d-flex flex-col"  :key="index" v-if="item.compteurNumber==compteur">
...
export default {
  ...
  methods: {
    recheachUsers() {
      axios.get("http://localhost:8082/api/users").then((response) => {
    ...
    updateUser(){
      ...
      axios.put("http:localhost:8082/api/user/"+this.compteur,
      data
      ).then((response)=>{
